...
// POST /api/admin/change-password -> Change administrator console password
router.post("/change-password", (req, res) => {
  ...
  try {
    const admin = db
      ...
      .get(adminId);
    if (!admin || !comparePassword(old_password, admin.password_hash)) {
      return res
        .status(401)
        .json({ error: "Incorrect current admin password." });
    }

    const hashed = hashPassword(new_password);
    db.prepare("UPDATE admins SET password_hash = ? WHERE admin_id = ?").run(
      hashed,
      adminId,
    );
  ...
  try {
    ...
    db.prepare(
      ...
      INSERT INTO profile_edit_requests (
        ...
        education_history, status, created_at
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Pending', ?)
    `,
    ).run(
...
router.get("/attendance/stats", (req, res) => {
  try {
    const date = (req.query.date || nowIso().slice(0, 10)).slice(0, 10);
    const { present } = db
      .prepare("SELECT COUNT(*) as present FROM attendance_log WHERE date = ?")
      .get(date);
    const { expected } = db
      .prepare(
        "SELECT COUNT(*) as expected FROM students WHERE status = 'Active'",
      )
      .get();
    res.json({ present, expected, date });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// GET /api/admin/attendance/absent?date=YYYY-MM-DD → absent active students
router.get("/attendance/absent", (req, res) => {
  try {
    const date = (req.query.date || nowIso().slice(0, 10)).slice(0, 10);
    const students = db
      .prepare(
        `
      SELECT s.student_id, s.name, s.phone, s.whatsapp, s.email,
             seats.seat_number, sa.start_time, sa.end_time
      FROM students s
      LEFT JOIN seat_allocations sa ON sa.student_id = s.student_id AND sa.active = 1
      LEFT JOIN seats ON seats.seat_id = sa.seat_id AND seats.active = 1
      WHERE s.status = 'Active'
        AND s.student_id NOT IN (
          SELECT student_id FROM attendance_log WHERE date = ?
        )
      ORDER BY s.name ASC
    `,
      )
...
// GET /api/admin/attendance?date=YYYY-MM-DD → present students
router.get("/attendance", (req, res) => {
  try {
    const raw = req.query.date || nowIso().slice(0, 10);
    const date = raw.slice(0, 10); // always truncate to YYYY-MM-DD
    const students = db
      .prepare(
        `
      SELECT s.student_id, s.name, s.phone, s.whatsapp, s.email,
...
// POST /api/admin/attendance/notify/:studentId → send absence email
router.post("/attendance/notify/:studentId", async (req, res) => {
  try {
    ...
    const { sendAbsenceNoticeEmail } = require("../services/emailService");
    await sendAbsenceNoticeEmail(student, date);
